...
class SklearnExtraTreesClassifier(SklearnModel.SklearnModel):
    """
    A wrapper for the scikit learn ExtraTreesClassifier model
    """
    def __init__(self, properties_dict):
        super(SklearnExtraTreesClassifier, self).__init__(properties_dict)

    def describe():
        description = "The ScikitLearn Extra Trees Ensemble Classifier. Documentation: http://scikit-learn.org/stable/modules/generated/sklearn.ensemble.ExtraTreesClassifier.html" 
    ...
    def summarize(self, file_path):
        ...
        with open(file_path + 'sklearn_extra_trees_feature_importances.txt', 'w') as f:
        ...
        try:
            ...
            for estimator in self._trained_model.estimators_:
                with open(file_path + 'sklearn_extra_trees_{0}.dot'.format(tree_index), 'w') as f:
                ...
                #Save pdf
                graph = pydotplus.graph_from_dot_file(file_path + 'sklearn_extra_trees_{0}.dot'.format(tree_index))
                graph.write_pdf(file_path + 'sklearn_extra_trees_{0}.pdf'.format(tree_index))
                tree_index += 1

            merger = PyPDF2.PdfFileMerger()
            for i in range(0, tree_index, 1):
                with open(file_path + 'sklearn_extra_trees_{0}.pdf'.format(i), 'rb') as f:
